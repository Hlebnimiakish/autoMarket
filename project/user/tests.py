...
def test_create_users_with_different_types(user_data, client):
    response = client.post(f'{BASE_URL}{reverse("registration")}',
                           json=user_data)
...
def test_user_can_read_update_delete_his_user_profile(unverified_user, client):
    response = client.get(f'{BASE_URL}{reverse("user")}',
    ...
    response = client.put(f'{BASE_URL}{reverse("user")}',
                          headers=unverified_user['headers'],
                          json=unverified_user['created_user'])
    assert response.status_code == 200
    assert response.json()['username'] == unverified_user['created_user']['username']
    response = client.delete(f'{BASE_URL}{reverse("user")}',
                             headers=unverified_user['headers'])
    assert response.status_code == 200
...
def test_created_user_can_log_in_and_refresh(user_data, client):
    client.post(f'{BASE_URL}{reverse("registration")}', json=user_data)
    response = client.post(f'{BASE_URL}{reverse("get-token")}',
                           json=user_data)
    refresh = response.json()['refresh']
    assert response.status_code == 200
    assert response.json()['access']
    response = client.post(f'{BASE_URL}{reverse("refresh-token")}',
...
def test_inactive_user_can_not_login_and_refresh(unverified_user, client):
    ...
    client.put(f'{BASE_URL}{reverse("user")}',
               headers=unverified_user['headers'],
               json=unverified_user['created_user'])
    response = client.post(f'{BASE_URL}{reverse("get-token")}',
...
def test_user_can_be_verified(unverified_user, client):
    ...
    response = client.put(f'{BASE_URL}{reverse("verification")}',
                          json=id_data)
    assert response.status_code == 200
...
def test_unverified_users_can_not_create_profile(unverified_user, dealer_profile, client):
    response = client.post(f'{BASE_URL}{reverse("dealer-creation")}',
                           json=dealer_profile,
                           headers=unverified_user['headers'])
...
def test_users_with_different_types_can_create_only_corresponding_profile(verified_user,
    ...
    for user_type, profile in {'dealer': dealer_profile,
        ...
        response = client.post(f'{BASE_URL}{reverse(url_name)}',
                               json=profile,
                               headers=verified_user['headers'])
        if user_type == str.lower(verified_user['created_user']['user_type']):
            assert response.status_code == 201
            url_name = f'{user_type}-profile'
            response = client.get(f'{BASE_URL}{reverse(url_name)}',
                                  headers=verified_user['headers'])
            assert response.status_code == 200
        else:
            assert response.status_code == 403
...
def test_dealer_seller_can_read_update_delete_his_type_profile(verified_user,
    ...
    url_name = f'{user_type}-creation'
    client.post(f'{BASE_URL}{reverse(url_name)}',
                json=profile_data,
                headers=verified_user['headers'])
    url_name = f'{user_type}-profile'
    response = client.get(f'{BASE_URL}{reverse(url_name)}',
    ...
    response = client.put(f'{BASE_URL}{reverse(url_name)}',
                          headers=verified_user['headers'],
                          json=profile_data)
    assert response.status_code == 200
    assert profile_data['name'] == response.json()['name']
    response = client.delete(f'{BASE_URL}{reverse(url_name)}',
                             headers=verified_user['headers'])
    assert response.status_code == 200
...
def test_buyer_can_read_update_delete_his_type_profile(verified_user,
    ...
    client.post(f'{BASE_URL}{reverse("buyer-creation")}',
                json=user_profile,
                headers=verified_user['headers'])
    response = client.get(f'{BASE_URL}{reverse("buyer-profile")}',
                          headers=verified_user['headers'])
    assert response.status_code == 200
    assert response.json()['is_active']
    profile_data = response.json()
    profile_data['firstname'] = f'PreDelete{uuid.uuid4()}'
    response = client.put(f'{BASE_URL}{reverse("buyer-profile")}',
                          headers=verified_user['headers'],
                          json=profile_data)
    assert response.status_code == 200
    assert profile_data['firstname'] == response.json()['firstname']
    response = client.delete(f'{BASE_URL}{reverse("buyer-profile")}',
                             headers=verified_user['headers'])
    assert response.status_code == 200
...
def test_dealer_can_view_other_users(all_users, all_profiles, client):
    ...
    for user_type in all_users.keys():
        ...
        response = client.get(f'{BASE_URL}{reverse(url_name, kwargs={"pk": 1})}',
                              headers=dealer['headers'])
        assert response.status_code == 200
...
def test_buyer_can_view_other_users(all_users, all_profiles, client):
    ...
    response = client.get(f'{BASE_URL}{reverse("dealer-list")}',
                          headers=buyer['headers'])
    assert response.status_code == 200
    assert response.json()[0]['id']
    response = client.get(f'{BASE_URL}{reverse("dealer-detail", kwargs={"pk": 1})}',
                          headers=buyer['headers'])
    assert response.status_code == 200
    assert response.json()['id']
...
def test_seller_can_view_other_users(all_users, all_profiles, client):
    ...
    response = client.get(f'{BASE_URL}{reverse("buyer-list")}',
                          headers=buyer['headers'])
    assert response.status_code == 403
    response = client.get(f'{BASE_URL}{reverse("buyer-detail", kwargs={"pk": 1})}',
                          headers=buyer['headers'])
    assert response.status_code == 403
...
def test_invalid_data_tests(user_data, dealer_profile, client):
    ...
    client.put(f'{BASE_URL}{reverse("verification")}', json=id_data)
    key = client.post(f'{BASE_URL}{reverse("get-token")}',
