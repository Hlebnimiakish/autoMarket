...
def select_park_to_buy_from(offer: OfferModel) -> dict | None:
...
@app.tasks
def task_make_deal_from_offer(offer: OfferModel):
    ...
    buyer = offer.creator
    selected_park = select_park_to_buy_from(offer)
    buyer_balance = Decimal(buyer.balance)
    if selected_park and selected_park['actual_price'] <= buyer_balance:
        make_offer_deal(selected_park, buyer)
    else:
        check_offer = OfferModel.objects.get_or_none(id=offer.pk)
        if check_offer:
            task_make_deal_from_offer.apply_async(check_offer, countdown=300)
